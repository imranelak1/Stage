...
function loadGeoJSON() {
    // Check if user is AREF admin and load only their region's provinces
    if (window.userManager && userManager.isUserArefAdmin()) {
        const userAref = userManager.getUserAref();
        
        // Load only provinces for this AREF region (no regions GeoJSON needed)
        fetch(`/t3shield/api/geojson/provinces/${userAref}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
                ...
                // Create provinces layer directly for AREF admin
                createArefProvincesLayer(data);
                
                // Update the map
                updateMap();
                
                // Apply AREF admin map behavior
                userManager.setupArefAdminMapBehavior();
            })
            .catch(error => {
                updateMap();
            });
    } else {
        // Super admin - load full regions GeoJSON and all provinces data
